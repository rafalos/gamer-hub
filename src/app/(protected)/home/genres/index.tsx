...
const Genres = async () => {
  ...
  return (
    <div
      ...
      before:h-10 before:w-10 before:absolute before:blur-md lg:after:hidden lg:before:hidden py-4'
    >
      {genres.map(({ id, name }) => (
        <div
          key={id}
          className='bg-white border b h-8 p-1 px-4 rounded-md hover:bg-accent hover:cursor-pointer transition-colors'
        >
          {name}
        </div>
      ))}
